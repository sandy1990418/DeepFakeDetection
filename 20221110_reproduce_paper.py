...
# dump(combine_color_and_hist_distance,"combine_color_and_hist_distance.sav")
# test=load("combine_color_and_hist_distance.sav",'r')
##################################################################
##################################################################
...
# ################################################################
# # ## video result 
# real_fold = ['deepfake_database/Celeb-DF/Celeb-DF-v1/Celeb-real','deepfake_database/Celeb-DF/Celeb-DF-v1/YouTube-real']
# fake_fold = 'deepfake_database/Celeb-DF/Celeb-DF-v1/Celeb-synthesis'

# video_df = list_video_df(fake_fold,real_fold)
# # video_df.head(5)
...
if __name__=="__main__":
    ...
    ##### 把整理好的資料進行特徵萃取 #######
    df_index = pd.read_pickle('block/df_index_balanced_20221028.pkl')
    tqdm.pandas()
    block = df_index['index'].progress_apply(lambda x : get_texture(df_index.iloc[x]))
    df_index = pd.concat([df_index,block],axis=1)
    df_index.to_pickle(f'block/df_index_balanced_main_reference_reproduce.pkl')
    del df_index
    gc.collect()
    
    main_reference = pd.read_pickle('block/df_index_balanced_main_reference_reproduce.pkl')
